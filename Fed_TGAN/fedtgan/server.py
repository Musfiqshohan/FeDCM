...
class Server(object):
    ...
    def model_update(self,base_model, clients_model, weights, iter):
        """
        :param base_model:
        :param clients_model:
        :param weights:
        ...
        for key in clients_model.keys():
            for name, param in clients_model[key][iter].items():  #aggregating layer weights of iter'th model of all clients
                new_model[name]= new_model[name] + clients_model[key][iter][name] * weights[key]
    ...
    def model_aggregate(self, clients_dis, clients_gen, weights):
        ...
        for iter in range(len(self.global_generator)):
            global_dis = self.model_update(self.global_discriminator[iter]['model'], clients_dis,weights, iter)
            global_gen = self.model_update(self.global_generator[iter]['model'], clients_gen, weights, iter)

            #update the global discriminator and generator
            self.global_discriminator[iter]['model'].load_state_dict(global_dis)
            self.global_generator[iter]['model'].load_state_dict(global_gen)

    def apply_activate(self, transformer, data, out_indx):

        """Apply proper activation function to the output of the generator."""
        data_t = []
        st = 0
        for key, column_info in transformer.output_info_dict.items():
    ...
    @torch.no_grad()
    def sample(self,n, transformer, conf, test_input, global_generator, allowed_vars):
        """
        :param n:
        :param transformer:
        :param conf:
        ...
        for i in range(steps):
            ...
            if len(inputs)>0:
                ...
                index = np.random.randint (data_size, size=conf['batch_size'])
                inputs = inputs[index]
                inputs = [torch.from_numpy(inputs.astype('float32')).to(self.device)]

            # inputs = []
            # for inp in global_generator['in_indx']:
            #     inputs.append(real[:, conf['in_indx'][inp][0]: conf['in_indx'][inp][1]])
            ...
            fake = global_generator['model']([fakez], inputs)
            fakeact = self.apply_activate(transformer, fake, global_generator['out_indx'])
        ...
        return transformer.inverse_transform(data, allowed_vars)
