...
class Client(object):
    ...
    def evaluate_dcm(self, args, label_generators, current_real_label, results, sample_size):
        ...
        with torch.no_grad():
            ...
            print('causal effect optimizing', args.max_intv)
    ...
    def train_batch(self, args, dcm, dcm_gopt, dcm_critic, dcm_dopt, obs_batch, intv_batch, optimize=None):
        ...
        #
        # Causal efffect loss: parameters: args.optimize, args.intv, args.opt_lambda
        if args.max_intv != []:
            optimize, intv, val, opt_lambda= args.max_intv

            if optimize == 'maximize':
                targAll = torch.nn.functional.one_hot(torch.ones(args.batch_size).to(torch.int64), num_classes=2).float().to(self.device)
            elif optimize == 'minimize':
                targAll = torch.nn.functional.one_hot(0 * torch.ones(args.batch_size).to(torch.int64),num_classes=2).float().to(self.device)
            else:
                raise RuntimeError('Invalid optimization')

            # do(Z=1)     # P(Y=1|do(Z=1)) to maximize;
            gen_intv = self.get_generated_labels(args, dcm, intervened={intv: val}, chosen_labels=['Y'],
                                            mini_batch=args.batch_size)['Y']

            loss1 = self.criterion(gen_intv, targAll)
            loss_g += opt_lambda * loss1
        #
    ...
    def intv_sample(self, n, conf): # graph-specific; need generalization
        ...
        if self.conf['data_name']=='frontdoor' or self.conf['data_name']=='nonid':
